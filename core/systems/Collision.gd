...
static func check_tile_collision(world_x: float, world_y: float, width: float, height: float, solid_layer: PackedInt32Array, map_cols: int) -> Dictionary:
	...
	var solid_min: int = GameConstants.COLLISION["SOLID_TILE_MIN"]
	var solid_max: int = GameConstants.COLLISION["SOLID_TILE_MAX"]
...
static func move_with_collision(player: Object, dx: float, dy: float, solid_layer: PackedInt32Array, map_cols: int) -> void:
	var tile_size: int = GameConstants.TILE["SIZE"]
	var hitbox_w: float = GameConstants.PLAYER["WIDTH"]
	var hitbox_h: float = GameConstants.PLAYER["HEIGHT"]
	var off_x: float = GameConstants.PLAYER["HITBOX_OFFSET_X"]
	var off_y: float = GameConstants.PLAYER["HITBOX_OFFSET_Y"]
