...
func _init(viewport_w: float, viewport_h: float, map_w_tiles: int, map_h_tiles: int) -> void:
	...
	var tile_size: float = GameConstants.TILE["SIZE"]
	w = viewport_w / scale
	h = viewport_h / scale
...
func follow(target_x: float, target_y: float) -> void:
	...
	var deadzone_x: float = GameConstants.CAMERA["DEADZONE_X"]
	var deadzone_y: float = GameConstants.CAMERA["DEADZONE_Y"]
	var lerp_factor: float = GameConstants.CAMERA["LERP"]
